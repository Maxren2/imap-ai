...
All twenty-one verified end-to-end against a real Gmail account, a real local Postgres instance, and (for AI matching/chat) a real local Ollama server — not just typechecked, with one exception: the actual unsubscribe action (one-click POST / manual link / mailto send) was deliberately not exercised against a real vendor, at the user's request, so that specific step is unverified beyond code review. Along the way this project caught real bugs the hard way more than once:
...
Not yet built, and unlikely to be — see [DESIGN.md](DESIGN.md) §34 (Risks) and the phase write-ups (§10-32) for the reasoning: response-time analytics and an action-log-based archived/deleted breakdown (inbox-zero's real versions either aren't fully local-mirror-based either, or depend on an external service this project doesn't use), AI-based category refinement (would mean far more Ollama load than this project's already-flaky local server can reliably take), and per-row OR condition editing (inbox-zero's own real model doesn't have this either — deliberately not adopted, not a gap). Genuinely still missing: reply/forward/draft-email/delete rule-action types (meaningfully higher-risk than label/archive/markRead/star — composing content, guarding against destructive mistakes), a way to cancel a running background job from the UI, and a compose/reply UI in general (there's no way to read or reply to a full message yet, only list-view snippets).

**UI visual design polish, backlogged not started**: imap-ai only ever adopted inbox-zero's shadcn *structure*, never its real design tokens or layout chrome. See [DESIGN.md](DESIGN.md) §33 for the full itemized list, researched both from inbox-zero's source and by actually running its real, already-built Docker image locally for a live comparison (its production deployment had stopped responding) — wrong-colored (shadcn-default rather than inbox-zero's real) design tokens, a "Mail route" warm-neutral palette inbox-zero scopes separately from the rest of its app that imap-ai doesn't replicate, missing chrome (grouped/labeled sidebar sections, a command palette, a sidebar-less focused Mail layout, an account-switcher menu), no custom font (inbox-zero uses Inter via `next/font/google`; imap-ai falls back to the system font), and a richer Assistant empty-state (clickable suggested-prompt pills, a chat-history dropdown) than imap-ai's current plain-text one.
