...
Not yet built (in progress, phased -- see DESIGN.md §10): the AI assistant chat, a real mail list/thread view, bulk unsubscribe, bulk archive, and analytics -- the sidebar nav already has slots for all of these, marked "Soon." Also still missing: reply/forward/draft-email/delete actions, and a UI for triggering a full backfill (still CLI-only via `npm run backfill`). See [DESIGN.md](DESIGN.md) for the full plan.
