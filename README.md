...
5. **Web app** (`npm run dev`, then http://localhost:3000) — a Next.js app reading live from the same Postgres mirror. Home lists the account and recent messages; **`/rules` is a full rules manager** — create/edit/delete rules (conditions, AI prompt, actions) through plain HTML forms, toggle enabled, and trigger a detection or action-execution run right from the browser (spawned as a background process, same commands as the CLI).
...
Not yet built: reply/forward/draft-email/delete actions, and a UI for triggering a full backfill (still CLI-only via `npm run backfill`; a GUI toggle is intentionally deferred). See [DESIGN.md](DESIGN.md) for the full plan.
