...
import type { EmailAccount, Rule } from "../generated/prisma/index.js";
...
/**
 * Runs every enabled rule belonging to one account. A shared IMAP
 * connection (opened by the caller, once, only if some rule here actually
 * needs body content) is reused across every AI-evaluated rule in this
...
async function runAccount(account: EmailAccount): Promise<void> {
  ...
  // touches the network at all. One connection per account, never shared
  // across accounts -- it only ever sees that account's own mailbox.
  let imapClient: ImapFlow | undefined;
  let releaseLock: (() => void) | undefined;
  if (needsImap && ollamaConfig) {
    imapClient = await connectAccountImap(account);
    const lock = await imapClient.getMailboxLock("INBOX");
    releaseLock = () => lock.release();
  }

  try {
    await runRulesForAccount(rules, imapClient, ollamaConfig);
  } finally {
    releaseLock?.();
